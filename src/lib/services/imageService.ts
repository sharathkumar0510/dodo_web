...
export const imageService = {
  ...
  getImagesByServiceId: async (serviceId: string): Promise<ImageAsset[]> => {
    const cacheKey = `images-service-${serviceId}`;
    ...
    try {
      const response = await imageApi.getImagesByServiceId(serviceId);
      ...
      console.error(`Error fetching images for service ${serviceId}:`, error);
      return [];
    }
  },

  // Get images by category ID
  getImagesByCategoryId: async (categoryId: string): Promise<ImageAsset[]> => {
    const cacheKey = `images-categoryId-${categoryId}`;
    ...
    try {
      const response = await imageApi.getImagesByCategoryId(categoryId);
  ...
  getImagesByProfessionalId: async (professionalId: string): Promise<ImageAsset[]> => {
    const cacheKey = `images-professional-${professionalId}`;
    ...
    try {
      const response = await imageApi.getImagesByProfessionalId(professionalId);
      ...
      console.error(`Error fetching images for professional ${professionalId}:`, error);
      return [];
    }
  },
